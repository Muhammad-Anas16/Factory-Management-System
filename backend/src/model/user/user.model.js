...
export const updateUser = (user) => {
  const statment = db.prepare(`
        UPDATE users SET (
        name = ?, username = ?, role = ?, allowedPages = ?, canAdd = ?, canEdit = ?, canDelete = ?, status = ?, updatedAt = CURRENT_TIMESTAMP WHERE id = ?
    )`);
  return statment.run(
    user.name,
    user.username,
...
export const updateUserPassword = (id, hashedPassword) => {
  const statment = db.prepare(`
        UPDATE users SET password = ?, updatedAt = CURRENT_TIMESTAMP WHERE = ?
    `);
  return statment.run(hashedPassword, id);
};

export const deleteUser = (id) => {
  const statment = db.prepare(`DELETE FROM users WHERE id = ?`);
  return statment.run(id);
};
